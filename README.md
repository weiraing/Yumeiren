...
- **资源友好**：帧率上限默认 30 fps（限帧时丢弃多余帧，播放速度不变），4K60 素材内存 / 显存约 **−40%**；「限制逻辑核」默认开启（限前 4 个逻辑核），实测 1080p30 内存约 **−27%**、显存约 **−36%**、CPU 不变；长时间挂起自动卸载解码管线，恢复时跳回暂停位置。
...
- **素材已随仓库提供**：`data/` 下带了一套可直接用的示例素材（约 79 MB —— 4 个 Live2D 模型、图片图库、视频、19 套网页壁纸），构建时由 CMake 整个拷到程序目录。想换成自己的，直接替换 `data/` 下对应子目录即可：程序按**固定目录结构**扫描，不需要任何清单文件。其中第三方模型 / 图片 / 视频的版权归各自作者，**对外分发或商用前请逐个确认授权**（见[致谢与许可](#-致谢与许可)）。
...
| `data/`（模型 / 图片 / 视频） | ⬜ | **随仓库提供**，正常无需自备。若被删空则图库为空、看板娘无模型 |
...
> ⚠️ `.gitignore` 排除了 `third_party/`、`tools/`、`docs/` 与 `build*/`（`third_party/` 与
...
`data/` 已经随仓库提供，通常不用管；**只有想要真 Live2D 看板娘、或想换掉示例素材时**才需要动这里。目录结构是固定的：
...
| 代码行数 | **16,729** 行 |
| 模块数 | **10** 个 |
| 入库文件 | **316** 个（`git ls-files` 计数；其中 `data/` 素材 169 个，另有 `.gitignore` 排除的 SDK / 探针） |
| 设计文档 | **25** 篇（`docs/`） |
| 构建目标 | 2 个产品目标 + 2 个探针 |
| 随仓库提供的 Live2D 模型 | **4** 个（`data/models/`） |
...
- **Live2D 许可提醒**：Cubism SDK 不是开源软件，使用需遵守其专有许可（个人 / 小规模事业者有免费额度，发行前请确认最新条款）。
- **仓库里的素材都是第三方的**：`data/models/` 下的 4 个模型、`data/image/`、`data/video/`、`data/web/` 均来自第三方，**版权归各自作者，本项目未逐一取得授权**，随仓库提供只是为了克隆下来就能跑。使用它们需额外遵守**各作者**的使用条款（很多 Live2D 模型明确禁止商用）。要对外分发或商用，请先逐个确认授权，或者把相应子目录从 `data/` 删掉再发布 —— 素材缺失只会降级（图库为空 / 看板娘无模型），程序不会崩。
- **截图说明**：`static/` 下的效果图是本机实拍，其中展示的图片、视频与 Live2D 模型来自 `data/`（现已随仓库提供），版权归各自作者所有。
