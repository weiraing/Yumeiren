...
//
// **返回 false 的含义是「目录仍然存在」**，而**不是**「什么都没删」：本函数只把
// 「目标路径已经不存在」当作成功（返回码非零但东西确实没了的情况算成功，原因见
// .cpp 里的长注释，本机实测到的 SHFileOperation 怪癖）；反过来，失败时**可能已经
// 删掉了大部分内容**（实测最常见的是「文件全进了回收站、只剩顶层空目录」）。
// 所以调用方措辞不能说「模型文件未被改动」，只能说「没能删干净」，并且应当把
// error 里的原因原样转达给用户。
//
// 失败时把**中文原因**写进 error（可为 nullptr）。
