...
// TaskbarCreated)，而画面要能在两种后端间切换 —— Live2D 必须活在 QOpenGLWidget
// 的 GL 上下文里，占位动画则走 QPainter。两种宿主控件基类不同，没法靠继承统一，
// 所以窗口持有子视图，交互事件用事件过滤器收在窗口一处，视图零交互代码。
//
// 禁止：本类不得每帧生成 QImage/QPixmap，更不得 grab() 截图。
