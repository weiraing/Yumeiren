// 看板娘模型预览图缓存：命名、查找、落盘。
//
// 文件名 = 模型文件夹名(即 ModelInfo::id)；取图先按名字找，找不到才生成 —— 刻意没有
// 「自动失效」判断，换过贴图/动作的模型会一直显示旧图，必须走强制重建。
...
public:
    ...
    // 模型 id → 缓存 PNG 绝对路径。不检查文件是否存在、不创建目录，是纯函数。
    // id 为空/含分隔符/为 "."/".." 时返回空串 —— 绝不让畸形 id 写出缓存目录之外。
    ...
    static bool remove(const QString &modelId);
