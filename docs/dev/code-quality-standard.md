...
详见 `docs/dev/comment-standard.md`。

核心原则：
- 注释只解释"为什么"，不重复"做什么"
- **能一句话说完的，不写两句**；默认动作是「不写」
- 命名清晰的代码不需要注释
- 超过 3 行的论述移入 `docs/dev/design-notes-*.md`，源码只留一句结论
- 禁止历史叙述（「曾有一个 xxx()」「2026-xx-xx 已删除」）—— 那属于 commit message
...
1. 先检查是否有可复用的现有代码
2. 文件级注释说明模块职责（不超过 3 行）
3. 类级注释说明职责与边界（不超过 4 行，仅公共接口/资源管理/状态机/平台相关类需要）
4. 非直观原因、硬约束、平台坑必须写一行说明
5. 资源所有权明确
6. 退出时正确清理
7. 构建验证
